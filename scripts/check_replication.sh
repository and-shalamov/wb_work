...
# Использование: ./check_patroni_replication.sh <pod_name> <namespace> <patroni_version> [password]

set -e

# Проверка аргументов
if [ $# -lt 3 ]; then
    echo "Использование: $0 <pod_name> <namespace> <patroni_version> [password]"
...
# Функция для выполнения SQL запросов (без указания порта, так как выполняется внутри пода)
exec_sql() {
...
# Проверяем, находится ли база в режиме recovery (standby)
IS_STANDBY=$(exec_sql "SELECT pg_is_in_recovery();" 2>/dev/null || echo "f")
...
if [ "$IS_STANDBY" = "t" ]; then
    echo "База находится в режиме standby, pg_stat_replication недоступен"
else
    exec_sql "SELECT client_addr, state, sync_state, 
              ...
              FROM pg_stat_replication;" 2>/dev/null || echo "Ошибка при выполнении SQL запроса или нет репликации"
fi

# 4. Проверка лага репликации
echo -e "\n4. Лаг репликации (в байтах):"
if [ "$IS_STANDBY" = "t" ]; then
    echo "База находится в режиме standby, информация о лаге недоступна"
else
...
echo -e "\n5. Слоты репликации:"
if [ "$IS_STANDBY" = "t" ]; then
    echo "База находится в режиме standby, слоты репликации недоступны"
else
    exec_sql "SELECT slot_name, plugin, slot_type, database, active, 
              ...
              FROM pg_replication_slots;" 2>/dev/null || echo "Ошибка при выполнении SQL запроса или нет слотов репликации"
fi
...
exec_in_pod du -smx /home/postgres/pgdata/pgroot/data 2>/dev/null || echo "Ошибка при проверке размера data directory"

# 8. Проверка активности WAL
echo -e "\n8. Активность WAL:"
if [ "$IS_STANDBY" = "t" ]; then
    echo "База находится в режиме standby, WAL функции недоступны"
    ...
    exec_sql "SELECT pg_current_wal_lsn(), 
              ...
              pg_size_pretty(pg_wal_lsn_diff(pg_current_wal_lsn(), '0/0')) as total_wal_pretty;" 2>/dev/null || echo "Ошибка при выполнении SQL запроса"
fi

# 9. Статус получения WAL (для standby)
echo -e "\n9. Статус получения WAL:"
if [ "$IS_STANDBY" = "t" ]; then
    exec_sql "SELECT status, last_msg_send_time, last_msg_receipt_time,
              ...
              FROM pg_stat_wal_receiver;" 2>/dev/null || echo "Ошибка при выполнении SQL запроса или нет получателя WAL"
else
    echo "База не находится в режиме standby, получатель WAL не активен"
fi
...
exec_sql "SELECT spcname, 
          ...
          FROM pg_tablespace 
          WHERE spcname != 'pg_default';" 2>/dev/null || echo "Ошибка при выполнении SQL запроса или нет табличных пространств"
