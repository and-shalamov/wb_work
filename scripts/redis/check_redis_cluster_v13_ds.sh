...
check_all_sentinels() {
    echo -e "\n${PURPLE}${BOLD}Проверка всех сентинелов в кластере:${NC}"
    ...
    for pod in "${all_sentinel_pods[@]}"; do
        ...
        echo -e "\n${CYAN}Для проверки идентичности информации выполните:${NC}"
        echo -e "${CYAN}  ./check_redis_cluster.sh $POD_NAME $NAMESPACE${NC}"
        echo -e "${CYAN}# В выводе ищите раздел 'Проверка идентичности информации о мастерах на всех сентинелах'${NC}"
...
# Функция для проверки идентичности информации о мастерах на всех сентинелах
check_sentinel_master_consistency() {
    echo -e "\n${PURPLE}${BOLD}Проверка идентичности информации о мастерах на всех сентинелах:${NC}"
    ...
    echo -e "${CYAN}  Найдено сентинелов: ${#sentinel_pods[@]}${NC}"
    
    # Собираем информацию о мастерах с каждого сентинела
    local master_info_array=()
    local sentinel_status_array=()
    
    for sentinel_pod in "${sentinel_pods[@]}"; do
        echo -e "  ${CYAN}Получение информации с $sentinel_pod...${NC}"
        
        # Проверяем статус пода
        local pod_status=$(kubectl get pod -n $NAMESPACE $sentinel_pod -o jsonpath='{.status.phase}' 2>/dev/null || echo "Unknown")
        if [ "$pod_status" != "Running" ]; then
            echo -e "    ${RED}Пропуск: под $sentinel_pod не в состоянии Running ($pod_status)${NC}"
            sentinel_status_array+=("$sentinel_pod: НЕДОСТУПЕН ($pod_status)")
            continue
        fi
        
        # Получаем информацию о сентинеле
        local sentinel_info
        if [ -n "$REDIS_PASSWORD" ]; then
            sentinel_info=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -a '$REDIS_PASSWORD' -p 26379 info sentinel 2>&1" 2>/dev/null || echo "ERROR")
            sentinel_info=$(echo "$sentinel_info" | grep -v "Warning: Using a password")
        else
            sentinel_info=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -p 26379 info sentinel 2>&1" 2>/dev/null || echo "ERROR")
        fi
        
        if [[ "$sentinel_info" == *"ERROR"* ]] || [ -z "$sentinel_info" ]; then
            echo -e "    ${RED}Ошибка при получении информации с $sentinel_pod${NC}"
            sentinel_status_array+=("$sentinel_pod: ОШИБКА")
            continue
        fi
        
        # Извлекаем строки о мастерах (master0, master1 и т.д.)
        local master_lines=$(echo "$sentinel_info" | grep -E "^master[0-9]+:" | sort)
        
        if [ -z "$master_lines" ]; then
            echo -e "    ${YELLOW}Нет информации о мастерах на $sentinel_pod${NC}"
            sentinel_status_array+=("$sentinel_pod: НЕТ МАСТЕРОВ")
            master_info_array+=("$sentinel_pod: НЕТ МАСТЕРОВ")
        else
            # Форматируем информацию о мастерах
            local formatted_info=""
            while IFS= read -r line; do
                # Извлекаем основные параметры
                local master_num=$(echo "$line" | cut -d: -f1)
                local address=$(echo "$line" | grep -o "address=[^,]*" | cut -d= -f2)
                local sentinels=$(echo "$line" | grep -o "sentinels=[^,]*" | cut -d= -f2)
                local slaves=$(echo "$line" | grep -o "slaves=[^,]*" | cut -d= -f2)
                local status=$(echo "$line" | grep -o "status=[^,]*" | cut -d= -f2)
                
                formatted_info+="$master_num: address=$address, sentinels=$sentinels, slaves=$slaves, status=$status; "
            done <<< "$master_lines"
            
            # Удаляем последнюю точку с запятой и пробел
            formatted_info="${formatted_info%; }"
            
            echo -e "    ${GREEN}Информация о мастерах:${NC}"
            echo -e "      ${CYAN}$formatted_info${NC}"
            
            master_info_array+=("$sentinel_pod: $formatted_info")
            sentinel_status_array+=("$sentinel_pod: OK")
        fi
    done
    
    # Проверяем идентичность информации на всех сентинелах
    echo -e "\n${CYAN}  Анализ согласованности информации:${NC}"
    
    if [ ${#master_info_array[@]} -lt 2 ]; then
        echo -e "    ${YELLOW}Недостаточно данных для сравнения (требуется минимум 2 доступных сентинела)${NC}"
        return 0
    fi
    
    # Берем первую запись как эталон
    local reference_info="${master_info_array[0]}"
    local reference_pod=$(echo "$reference_info" | cut -d: -f1)
    
    echo -e "    ${CYAN}Эталон (с $reference_pod):${NC}"
    echo -e "      ${GREEN}$reference_info${NC}"
    
    local all_match=true
    local mismatch_count=0
    
    for ((i=1; i<${#master_info_array[@]}; i++)); do
        local current_info="${master_info_array[$i]}"
        local current_pod=$(echo "$current_info" | cut -d: -f1)
        
        if [ "$current_info" = "$reference_info" ]; then
            echo -e "    ${GREEN}✓ $current_pod: совпадает${NC}"
        else
            echo -e "    ${RED}✗ $current_pod: НЕ СОВПАДАЕТ${NC}"
            echo -e "      ${YELLOW}Отличается: $current_info${NC}"
            all_match=false
            ((mismatch_count++))
        fi
    done
    ...
    # Вывод итогов
    echo -e "\n${PURPLE}${BOLD}Итоги проверки идентичности:${NC}"
    
    if [ "$all_match" = true ]; then
        echo -e "${GREEN}  ✓ ВСЕ СЕНТИНЕЛЫ ИМЕЮТ ИДЕНТИЧНУЮ ИНФОРМАЦИЮ О МАСТЕРАХ${NC}"
        echo -e "${GREEN}  ✓ Конфигурация синхронизирована на всех узлах${NC}"
        
        # Показываем общую информацию о мастерах
        if [ -n "$reference_info" ] && [ "$reference_info" != *"НЕТ МАСТЕРОВ"* ]; then
            echo -e "\n${CYAN}  Общая информация о мастерах в кластере:${NC}"
            
            # Разбираем строку с информацией о мастерах
            local master_lines=$(echo "$reference_info" | sed 's/; /\n/g')
            
            while IFS= read -r master_line; do
                if [ -n "$master_line" ]; then
                    local master_num=$(echo "$master_line" | cut -d: -f1)
                    local address=$(echo "$master_line" | grep -o "address=[^,]*" | cut -d= -f2)
                    local sentinels=$(echo "$master_line" | grep -o "sentinels=[^,]*" | cut -d= -f2)
                    local slaves=$(echo "$master_line" | grep -o "slaves=[^,]*" | cut -d= -f2)
                    local status=$(echo "$master_line" | grep -o "status=[^,]*" | cut -d= -f2)
                    
                    # Извлекаем IP мастера
                    local master_ip=$(echo "$address" | cut -d: -f1)
                    local master_port=$(echo "$address" | cut -d: -f2)
                    
                    # Получаем имя пода мастера
                    local master_pod=$(get_pod_name_by_ip "$master_ip")
                    
                    echo -e "    ${PURPLE}$master_num:${NC}"
                    echo -e "      ${CYAN}Мастер: $master_pod ($address)${NC}"
                    echo -e "      ${CYAN}Статус: $status${NC}"
                    echo -e "      ${CYAN}Сентинелов: $sentinels${NC}"
                    echo -e "      ${CYAN}Реплик: $slaves${NC}"
                    
                    # Дополнительная информация о мастере
                    echo -e "      ${YELLOW}Подробная информация:${NC}"
                    
                    # Получаем детальную информацию о мастере с первого рабочего сентинела
                    local first_sentinel="${sentinel_pods[0]}"
                    local master_name
                    
                    # Получаем имя мастера
                    if [ -n "$REDIS_PASSWORD" ]; then
                        master_name=$(kubectl exec -n $NAMESPACE $first_sentinel -- sh -c "redis-cli -a '$REDIS_PASSWORD' -p 26379 sentinel masters 2>&1" 2>/dev/null | grep "name" | head -1 | awk -F '"' '{print $2}')
                    else
                        master_name=$(kubectl exec -n $NAMESPACE $first_sentinel -- sh -c "redis-cli -p 26379 sentinel masters 2>&1" 2>/dev/null | grep "name" | head -1 | awk -F '"' '{print $2}')
                    fi
                    
                    if [ -n "$master_name" ]; then
                        echo -e "      ${YELLOW}Имя мастера: $master_name${NC}"
                        
                        # Получаем информацию о кворуме
                        local quorum_info
                        if [ -n "$REDIS_PASSWORD" ]; then
                            quorum_info=$(kubectl exec -n $NAMESPACE $first_sentinel -- sh -c "redis-cli -a '$REDIS_PASSWORD' -p 26379 sentinel master $master_name 2>&1" 2>/dev/null | grep "quorum")
                        else
                            quorum_info=$(kubectl exec -n $NAMESPACE $first_sentinel -- sh -c "redis-cli -p 26379 sentinel master $master_name 2>&1" 2>/dev/null | grep "quorum")
                        fi
                        
                        if [ -n "$quorum_info" ]; then
                            echo -e "      ${YELLOW}Кворум: $quorum_info${NC}"
                        fi
                    fi
                fi
            done <<< "$master_lines"
        ...
        echo -e "${RED}  ⚠️  ОБНАРУЖЕНЫ РАСХОЖДЕНИЯ В ИНФОРМАЦИИ О МАСТЕРАХ${NC}"
        echo -e "${RED}  Количество несовпадающих сентинелов: $mismatch_count из ${#master_info_array[@]}${NC}"
        
        echo -e "\n${YELLOW}  Возможные причины расхождений:${NC}"
        echo -e "${YELLOW}    1. Разные версии Redis Sentinel на узлах${NC}"
        echo -e "${YELLOW}    2. Проблемы с синхронизацией конфигурации между сентинелами${NC}"
        echo -e "${YELLOW}    3. Сетевые проблемы между сентинелами${NC}"
        echo -e "${YELLOW}    4. Разное время обновления информации на сентинелах${NC}"
        echo -e "${YELLOW}    5. Ошибки в настройках сентинелов${NC}"
        
        echo -e "\n${YELLOW}  Рекомендуемые действия:${NC}"
        echo -e "${CYAN}    1. Проверьте логи всех сентинелов на предмет ошибок синхронизации${NC}"
        echo -e "${CYAN}    2. Убедитесь, что все сентинелы могут общаться друг с другом${NC}"
        echo -e "${CYAN}    3. Проверьте версии Redis на всех узлах${NC}"
        echo -e "${CYAN}    4. Перезапустите проблемные сентинелы${NC}"
        echo -e "${CYAN}    5. Выполните принудительную синхронизацию:${NC}"
        echo -e "${CYAN}       redis-cli -p 26379 sentinel reset mymaster${NC}"
        echo -e "${CYAN}    6. Проверьте конфигурационные файлы сентинелов${NC}"
        
        # Показываем статус каждого сентинела
        echo -e "\n${CYAN}  Статус сентинелов:${NC}"
        for status_info in "${sentinel_status_array[@]}"; do
            if [[ "$status_info" == *"OK"* ]]; then
                echo -e "    ${GREEN}$status_info${NC}"
            elif [[ "$status_info" == *"НЕТ МАСТЕРОВ"* ]]; then
                echo -e "    ${YELLOW}$status_info${NC}"
            else
                echo -e "    ${RED}$status_info${NC}"
            fi
        done
        
        # Рекомендуем проверить конкретные сентинелы
        echo -e "\n${YELLOW}  Команды для детальной проверки:${NC}"
        for sentinel_pod in "${sentinel_pods[@]}"; do
            echo -e "${CYAN}    kubectl exec -n $NAMESPACE $sentinel_pod -- redis-cli -p 26379 info sentinel | grep -E '^master[0-9]+:'${NC}"
        done
    fi
    
    # Проверка временных меток обновления информации
    echo -e "\n${CYAN}  Проверка временных меток обновления информации:${NC}"
    ...
    for sentinel_pod in "${sentinel_pods[@]}"; do
        ...
        local info_refresh
        if [ -n "$REDIS_PASSWORD" ]; then
            info_refresh=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -a '$REDIS_PASSWORD' -p 26379 info sentinel 2>&1" 2>/dev/null | grep "info-refresh" | head -1)
        else
            info_refresh=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -p 26379 info sentinel 2>&1" 2>/dev/null | grep "info-refresh" | head -1)
        fi
        
        if [ -n "$info_refresh" ]; then
            echo -e "    ${CYAN}$sentinel_pod: $info_refresh${NC}"
        fi
    done
...
# В функции check_sentinel, перед выводом дополнительных команд добавьте:

    # 8. Проверка идентичности информации о мастерах на всех сентинелах
    check_sentinel_master_consistency
    
    # 9. Подробная информация о мастерах
    show_detailed_sentinel_master_info
...
# Функция для вывода подробной информации о мастерах с каждого сентинела
show_detailed_sentinel_master_info() {
    echo -e "\n${PURPLE}${BOLD}Подробная информация о мастерах с каждого сентинела:${NC}"
    
    # Получаем все поды сентинелов
    local all_sentinel_pods=($(kubectl get pods -n $NAMESPACE --no-headers -o custom-columns=":metadata.name" | grep "rfs-" || true))
    local sentinel_pods=()
    ...
    for sentinel_pod in "${sentinel_pods[@]}"; do
        ...
        # Получаем полную информацию о сентинеле
        local sentinel_info
        if [ -n "$REDIS_PASSWORD" ]; then
            sentinel_info=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -a '$REDIS_PASSWORD' -p 26379 info sentinel 2>&1" 2>/dev/null || echo "ERROR")
            sentinel_info=$(echo "$sentinel_info" | grep -v "Warning: Using a password")
        ...
        # Выводим информацию о мастерах в удобном формате
        echo -e "    ${GREEN}Информация о мастерах:${NC}"
        
        # Разделяем вывод на строки и обрабатываем каждую
        echo "$sentinel_info" | while IFS= read -r line; do
            if [[ "$line" =~ ^master[0-9]+: ]]; then
                # Разбираем строку master
                local master_num=$(echo "$line" | cut -d: -f1)
                local params=$(echo "$line" | cut -d: -f2-)
                
                # Извлекаем ключевые параметры
                local address=$(echo "$params" | grep -o "address=[^,]*" | cut -d= -f2)
                local sentinels=$(echo "$params" | grep -o "sentinels=[^,]*" | cut -d= -f2)
                local slaves=$(echo "$params" | grep -o "slaves=[^,]*" | cut -d= -f2)
                local status=$(echo "$params" | grep -o "status=[^,]*" | cut -d= -f2)
                ...
                local last_ping_sent=$(echo "$params" | grep -o "last-ping-sent=[^,]*" | cut -d= -f2)
                local last_ok_ping_reply=$(echo "$params" | grep -o "last-ok-ping-reply=[^,]*" | cut -d= -f2)
                local down_after_milliseconds=$(echo "$params" | grep -o "down-after-milliseconds=[^,]*" | cut -d= -f2)
                
                if [ -n "$last_ping_sent" ]; then
                    echo -e "        ${YELLOW}Последний ping отправлен: $last_ping_sent мс назад${NC}"
                fi
                if [ -n "$last_ok_ping_reply" ]; then
                    echo -e "        ${YELLOW}Последний успешный reply: $last_ok_ping_reply мс назад${NC}"
                fi
                if [ -n "$down_after_milliseconds" ]; then
                    echo -e "        ${YELLOW}Таймаут down: $down_after_milliseconds мс${NC}"
                fi
            elif [[ "$line" =~ ^sentinel_masters: ]] || [[ "$line" =~ ^sentinel_tilt: ]] || [[ "$line" =~ ^sentinel_running_scripts: ]]; then
                # Выводим общую информацию о сентинеле
                echo -e "    ${GREEN}$line${NC}"
            fi
        done
        
        # Также получаем информацию через команду SENTINEL MASTERS для дополнительной проверки
        echo -e "    ${YELLOW}Проверка через SENTINEL MASTERS:${NC}"
        local sentinel_masters
        if [ -n "$REDIS_PASSWORD" ]; then
            sentinel_masters=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -a '$REDIS_PASSWORD' -p 26379 sentinel masters 2>&1" 2>/dev/null || echo "ERROR")
            sentinel_masters=$(echo "$sentinel_masters" | grep -v "Warning: Using a password")
        else
            sentinel_masters=$(kubectl exec -n $NAMESPACE $sentinel_pod -- sh -c "redis-cli -p 26379 sentinel masters 2>&1" 2>/dev/null || echo "ERROR")
        fi
        
        if [[ "$sentinel_masters" != *"ERROR"* ]]; then
            local master_count=$(echo "$sentinel_masters" | grep -c "name")
            echo -e "      ${GREEN}Найдено мастеров: $master_count${NC}"
            
            if [ "$master_count" -gt 0 ]; then
                # Выводим имена мастеров
                echo "$sentinel_masters" | grep "name" | while read line; do
                    local master_name=$(echo "$line" | awk -F '"' '{print $2}')
                    echo -e "      ${CYAN}Мастер: $master_name${NC}"
                done
            fi
...
if [ "$POD_TYPE" = "redis" ]; then
    ...
    if [ ${#sentinel_pods[@]} -gt 0 ]; then
        ...
        for sentinel_pod in "${sentinel_pods[@]:0:3}"; do  # Показываем первые 3
            ...
            echo -e "${YELLOW}Проверка идентичности информации на сентинелах:${NC}"
            echo -e "  ${CYAN}./check_redis_cluster.sh rfs-<имя-сентинела> $NAMESPACE${NC}"
            echo -e "  ${CYAN}# В выводе ищите раздел 'Проверка идентичности информации о мастерах на всех сентинелах'${NC}"
